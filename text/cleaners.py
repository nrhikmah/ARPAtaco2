...
def get_arpabet(word): # https://github.com/keithito/tacotron/blob/master/datasets/datafeeder.py
...
def cmudict_cleaners(text):
  '''Kind of like english_cleaners but with CMUDICT so its AWEOSME!!!!'''
  text = ' '.join([get_arpabet(word) for word in _arpabet_split_re.findall(text)])
  # text = convert_to_ascii(text)
  # text = lowercase(text)
  # text = expand_numbers(text)
  # text = expand_abbreviations(text)
